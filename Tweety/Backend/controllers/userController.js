...
export const Login = async (req, res) => {
  try {
    ...
    const tokenData = {
      userId: user._id, // Assuming your user model has _id field
      email: user.email, // You can add more data to token if needed
    };
    ...
    return res.status(201).json({
      ...
      user, // Assuming your user model has a username field
      success: true,
    });
  } catch (error) {
    console.log(error);
...
export const bookmarks = async (req, res) => {
  try {
    ...
    const user = await User.findById(loggedInUserId); // Correcting 'findbyId' to 'findById'
